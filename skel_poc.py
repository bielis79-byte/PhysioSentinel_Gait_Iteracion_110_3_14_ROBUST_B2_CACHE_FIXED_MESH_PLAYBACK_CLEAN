...
def _load_private_skel_bundle_auto():
    """V110.3.14 · Robust Backblaze B2 private loader + persistent runtime cache.
        ...
        if s3_url:
            try:
                ...
                with open(tmp,'wb') as f:
                    cli.download_fileobj(cfg['B2_BUCKET'],cfg['B2_FILE'],f)
...
def _persist_manual_bundle_runtime_cache(raw: bytes, preferred_name: str=''):
    """V110.3.14: una carga manual válida alimenta la misma caché estable.
...
def _acquisition_mode_gate_142(rmse_xy, rmse_z, audit, acquisition_mode, *, sanity_ok=True, unified_ok=True, xy_limit=0.38):
    """V110.3.13 · Puerta anatómica gobernada EXPLÍCITAMENTE por la modalidad de adquisición.

    No infiere el modo a partir de depth_weight ni de la existencia de XYZ.
    Modos admitidos:
      - monocular_depth_safe: una cámara frontal/posterior; Z inferida y no bloqueante.
      - biplanar_estimated: frontal+ lateral sin triangulación métrica; Z estimada y no bloqueante,
        pero se conserva auditoría anatómica dura y consistencia de cadenas.
      - 3d_strict: triangulación/calibración válida; XY y Z son bloqueantes.
    """
    ...
    hard_flags=[f for f in flags if not f.startswith(angular_prefixes)]
    ...
    if hard_flags: reasons.extend(hard_flags)
    if strict and (not ax.get('ok',False)):
        for f in flags:
    ...
    return {
        ...
        'ignored_depth_flags':[f for f in flags if f.startswith(angular_prefixes)] if depth_safe else [],
        'sanity_ok':bool(sanity_ok),'unified_ok':bool(unified_ok),'reasons':reasons,
        'rule':(
            'Monocular: XY + límites + coherencia; Z inferida informativa' if mode=='monocular_depth_safe' else
            'Biplanar estimado: XY + límites + coherencia; Z estimada informativa' if mode=='biplanar_estimated' else
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    return {'version':'110.3.13','retargeting_mode':'official_skel24_peripheral_refinement','scale':float(scale.cpu()),
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.14 · reproductor HTML/JS explícito para la malla SKEL.
    ...
    try:
        ...
        vv=np.round(V,5).tolist(); ff=F[:,:3].tolist(); jj=np.round(J,5).tolist() if J is not None else []
        ...
        #counter{{font-weight:700;min-width:100px}}#slider{{flex:1;min-width:280px}}#plot{{width:100%;height:690px}}</style>
        <script>{plotly_js}</script></head><body><div id='bar'><button onclick='playMesh(0.5)'>▶ 0.5×</button><button onclick='playMesh(1)'>▶ 1×</button><button onclick='playMesh(2)'>▶ 2×</button><button onclick='pauseMesh()'>⏸ Pausa</button><span id='counter'>Frame 1/{len(frame_ids)}</span><input id='slider' type='range' min='0' max='{max(0,len(frame_ids)-1)}' value='0' step='1'></div><div id='plot'></div>
        <script>
        const P={payload}; const gd=document.getElementById('plot'); const slider=document.getElementById('slider'); const counter=document.getElementById('counter');
        let idx=0, playing=false, speed=1, last=0, raf=0;
        ...
        const layout={{title:'V110.3.14 · SKEL ANIMATED ANATOMICAL MESH · marcha 3D',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}},legend:{{orientation:'h'}}}};
        ...
        st.caption("V110.3.14 · reproductor independiente de fig.frames. El contador debe avanzar al pulsar ▶; los 75 frames del NPZ científico no se modifican.")
    except Exception as exc:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.markdown("**Modelo SKEL privado · V110.3.14 CPU + Robust B2 Private Loader + Persistent Runtime Cache + Fixed Mesh Playback**")
    ...
    st.caption("V110.3.14 resuelve el modelo en este orden: CACHE HIT → B2 AUTH → descarga privada S3/Native → MANUAL FALLBACK. Una carga manual válida también queda en la caché `/tmp/physiosentinel_skel_b2_cache_v1_1` durante la vida de la instancia.")
        ...
        with st.spinner("Instanciando SKEL y ejecutando el primer forward CPU (1 frame)…"):
            ...
            try:
                ...
                try:
                    ...
                    export={
                        "version":"110.3.13","frame":int(best_i)+1,"gender":gender,"rmse_before":fit["rmse_before"],"rmse_after":fit["rmse_after"],
                    ...
                    st.markdown("### V110.3.13 · Propagación temporal 1→75")
                    ...
                    if st.button("🧍▶️ Procesar 75 frames + generar malla anatómica SKEL",type="primary",use_container_width=True,key="v110_3_13_run_sequence",disabled=not frame_gate_ok):
                        ...
                        try:
                            st.session_state.pop("v110_3_13_mesh_sequence",None)
                            st.session_state.pop("v110_3_13_mesh_audit",None)
                            ...
                            st.session_state["v110_3_13_sequence"]=seq
                            ...
                            if len(oks_auto)>=max(2,int(0.9*expected)):
                                ...
                                try:
                                    ...
                                    st.session_state["v110_3_13_mesh_sequence"]=mesh_seq
                                    st.session_state["v110_3_13_mesh_audit"]=mesh_audit
                    ...
                    if isinstance(seq,dict) and seq.get("frames"):
                        ...
                        st.download_button("⬇️ Descargar secuencia SKEL V110.3.13 (JSON)",json.dumps(_json_safe_134(export_seq),ensure_ascii=False).encode("utf-8"),"V110_3_13_SKEL_sequence.json","application/json",use_container_width=True)
                        ...
                        st.markdown("### V110.3.14 · SKEL ANIMATED ANATOMICAL MESH")
                        ...
                        if st.button("🔁 Regenerar malla anatómica SKEL",use_container_width=True,key="v110_3_13_build_skin_mesh"):
                            ...
                            try:
                                ...
                                if abs(float(mesh_seq.get("scale",0))-float(seq.get("scale",0)))>1e-7:
                                    raise RuntimeError("La escala de la malla no coincide con la secuencia temporal activa.")
                                st.session_state["v110_3_13_mesh_sequence"]=mesh_seq
                                st.session_state["v110_3_13_mesh_audit"]=_audit_skel_animated_mesh(mesh_seq)
                        ...
                        mesh_seq=st.session_state.get("v110_3_13_mesh_sequence")
                        if isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray):
                            ...
                            _ma=st.session_state.get("v110_3_13_mesh_audit") or _audit_skel_animated_mesh(mesh_seq)
                            a1,a2,a3,a4=st.columns(4)
                            ...
                            st.download_button("⬇️ Descargar SKEL Mesh Walker V110.3.14 (NPZ)",_mesh_sequence_npz_bytes(mesh_seq),"V110_3_14_SKEL_mesh_sequence.npz","application/octet-stream",use_container_width=True)
                            ...
                            st.markdown('### V110.3.13 · Validación sincronizada Side-by-Side')
                        ...
                        if frame_gate_ok:
                            st.info("✅ El frame semilla está validado. Pulsa el botón anterior para resolver frames 2→75; después V110.3.13 habilitará la malla corporal animada.")
                        else:
